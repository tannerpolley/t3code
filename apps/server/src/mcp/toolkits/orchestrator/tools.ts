...
const TaskStatusTool = Tool.make("task_status", {
  description:
    "Read a T3-owned delegated task created by this parent thread. childRunId identifies the original delegated run. workState distinguishes working, waiting_for_children, and result_available; a completed turn with live nested work is not a completed task. summary is the task's latest reported result, including provider errors on failure. A later turn this thread sends the child reopens the task: status and workState follow that turn, and summary keeps the previous result until the turn reports back and replaces it. hasPendingChildRuns reports later queued or executing turns; latestTerminal* provides later non-monitor turn results. waitingOnUser is present while the child waits on a question or approval for the user. Reading a terminal result acknowledges its automatic parent delivery.",
